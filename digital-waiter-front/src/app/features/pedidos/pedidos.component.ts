...
<section class="panel order-table"><div class="table-head-row"><b>Pedido</b><b>Origen</b><b>Responsable</b><b>Hora</b><b>Total</b><b>Estado</b><b>Acción</b></div>@for(o of store.orders();track o.id){<div class="table-data-row"><b>#{{o.id.toString().padStart(3,'0')}}</b><span>{{store.place(o)}}</span><span class="waiter-name">♙ {{o.customer||o.waiter||'Sin asignar'}}</span><span>{{o.createdAt|date:'shortTime'}}</span><strong>{{store.total(o)|currency:'USD'}}</strong><em>{{label(o.status)}}</em>@if(canAdvance(o)){<button class="small-button" (click)="advance(o)">{{action(o.status)}}</button>}@else{<span>—</span>}</div>}@empty{<p class="empty">No existen pedidos.</p>}</section>
...
export class PedidosComponent {
  ...
  canAdvance(o:Order){const role=this.store.session()?.role;return (role==='Administrador'&&o.status!=='paid')||(role==='Cocina'&&['new','preparing'].includes(o.status))||(role==='Mesero'&&o.status==='ready')||(role==='Cajero'&&(o.status==='payment'||(o.status==='ready'&&o.modality==='pickup')))}
